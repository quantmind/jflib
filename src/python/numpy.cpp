...
//#define WITHOUT_UBLAS_VECTOR
#define WITHOUT_NUMPY_TRACE

#include <jflib/python/numpy/all.hpp>
#ifndef WITHOUT_UBLAS_VECTOR
#include <jflib/python/numpy/numpy_vector.hpp>
#endif
...
#ifndef WITHOUT_UBLAS_VECTOR
template<class T>
typename numpy_vector<T>::size_type unstrided_size(const numpy_vector<T> &v){ return v.size(); }

template<class T>
typename numpy_strided_vector<T>::size_type strided_size(const numpy_strided_vector<T> &v) { return v.size(); }
#endif
...
struct array_converter_base  {
    ...
    static void construct_invalid_ok(PyObject* obj, py::converter::rvalue_from_python_stage1_data* data) {
    ...
    static void construct_indirect(PyObject* obj, py::converter::rvalue_from_python_stage1_data* data) {
      void* storage = ((py::converter::rvalue_from_python_storage<target_type>*)data)->storage.bytes;
...
public:
	static void construct_strided(PyObject* obj,
...
const PyTypeObject *get_PyArray_Type()  { return &PyArray_Type; }
...
void register_indirect_array_converter()  {
    ...
    py::to_python_converter<RealTgtType, typename Converter::template indirect_to_python<RealTgtType> >();
}
...
template <class T>
static void convert_array_scalar(PyObject* obj, py::converter::rvalue_from_python_stage1_data* data) {
...
#	ifndef WITHOUT_UBLAS_VECTOR
	...
	register_indirect_array_converter<vec_converter, ublas::vector<T> >();
	//register_indirect_array_converter<vec_converter, ublas::bounded_vector<T, 2> >();
    //register_indirect_array_converter<vec_converter, ublas::bounded_vector<T, 3> >();
#	endif
	...
	register_indirect_array_converter<rm_mat_converter, ublas::matrix<T, ublas::row_major> >();

	// Matrices - column majoir
	...
	register_indirect_array_converter<cm_mat_converter, ublas::matrix<T, ublas::column_major> >();
...
void set_trace(bool t) {trace_conversion = t;}

void numpy_expose_converters() {
	//import_array();
	//expose_converters<npy_byte>();
	//expose_converters<npy_short>();
	//expose_converters<npy_ushort>();
	//expose_converters<npy_bool>();
...
#	ifndef WITHOUT_NUMPY_TRACE
	py::def("set_ublas_trace", set_trace);
#	endif
}
