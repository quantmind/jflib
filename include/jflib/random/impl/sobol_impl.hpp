...
#		include <qmlib/random/primpoly.hpp>
...
inline RealType
lowDiscrepacySequenceGenerator<RealType>::point(
	typename lowDiscrepacySequenceGenerator<RealType>::size_type n,
	typename lowDiscrepacySequenceGenerator<RealType>::size_type dim) const  {
    QM_REQUIRE(n >= 1, "Sample point must be positive")
    QM_REQUIRE(dim >= 1, "Dimension must be positive")
    QM_REQUIRE(dim <= this->dimension(),"out of dimension");
    while(m_sequence.size()<=n) this->nextSequence();
    return m_sequence[n-1][dim-1];
}


template<class RealType>
inline void HaltonGenerator<RealType>::nextSequence() const  {
	seed_type n = this->sequenceCounter() + 1;
	QM_REQUIRE(n > 0, "Period exceeded: overflow");
	samples sam(this->dimension());
	seed_type b,i,k;
	RealType f, h;
	for (i=0;i<this->dimension();++i){
	    h = 0;
	    b = constlist::primenum(i);
	    f = 1.;
	    k = n;
	    while (k)  {
	    	f /= b;
	...
	m_sequence.push_back(sam);
}
	
template<class RealType, int D>
inline Sobol<RealType,D>::Sobol(
		typename Sobol<RealType,D>::size_type n,
		typename Sobol<RealType,D>::size_type dim):lowDiscrepacySequenceGenerator(dim,0),m_lastsequence(dim)  {
	QM_REQUIRE(m_dimensionality < PPMT_MAX_DIM,"dimensionality exceeds the number of available primitive polynomials modulo two");
	//
	m_directionIntegers.resize(m_dimensionality);
	for(size_type i=0;i<m_dimensionality;i++) m_directionIntegers[i].resize(c_bits);
	//
	// initializes coefficient array of the k-th primitive polynomial
	// and degree of the k-th primitive polynomial
	integers     		degree(m_dimensionality);
	std::vector<long>   ppmt(m_dimensionality);
	// degree 0 is not used
	ppmt[0]   = 0;
	degree[0] = 0;
	size_type k, index;
	size_type currentDegree;
	for (k=1,index=0,currentDegree=1; k<m_dimensionality; k++,index++)  {
	...
	//
	// initializes m_bits direction integers for each dimension
	// and store them into directionIntegers_[dimensionality_][m_bits]
	//
	...
	int j;
	for (j=0; j<c_bits; j++)
		m_directionIntegers[0][j] = (1UL<<(c_bits-j-1));
		//
		size_type maxTabulated = m_dimensionality;
		...
		switch(D)  {
            ...
            case 0:
                maxTabulated=m_dimensionality;
                for (k=1; k<maxTabulated; k++) {
                    for (qm_Size l=1; l<=degree[k]; l++) {
                        m_directionIntegers[k][l-1] = 1UL;
                        m_directionIntegers[k][l-1] <<= (c_bits-l);
                    }
                }
                break;
            case 1:
                ...
                for (k=1; k<std::min(m_dimensionality, maxTabulated); k++) {
                    j = 0;
                    // 0UL marks coefficients' end for a given dimension
                    while (initializers[k-1][j] != 0UL) {
                        m_directionIntegers[k][j] = initializers[k-1][j];
                        m_directionIntegers[k][j] <<= (c_bits-j-1);
                        j++;
                    }
                }
                break;
            case 2:
                ...
                for (k=1; k<std::min(m_dimensionality, maxTabulated); k++) {
                    ...
                    while (SLinitializers[k-1][j] != 0UL) {
                        m_directionIntegers[k][j] = SLinitializers[k-1][j];
                        m_directionIntegers[k][j] <<= (c_bits-j-1);
                        j++;
                    }
                }
            ...
            case 3:
                ...
                for (k=1; k<std::min(m_dimensionality, maxTabulated); k++) {
                    ...
                    while (Linitializers[k-1][j] != 0UL) {
                        m_directionIntegers[k][j] = Linitializers[k-1][j];
                        m_directionIntegers[k][j] <<= (c_bits-j-1);
                        j++;
                    }
                }
        ...
        if (m_dimensionality>maxTabulated)
        {
            MersenneTwister uniformRng(seed);
            for (k=maxTabulated; k<m_dimensionality; k++) {
                for (qm_Size l=1; l<=degree[k]; l++) {
                    do {
                        // u is in (0,1)
                        qm_real u = uniformRng.nextUniform();
                        // the direction integer has at most the
                        // rightmost l bits non-zero
                        m_directionIntegers[k][l-1] =
                            (qm_uns_long)(u*(1UL<<l));
                    } while (!(m_directionIntegers[k][l-1] & 1UL));
                    // iterate until the direction integer is odd
                    // that is it has the rightmost bit set

                    // shifting m_bits-l bits to the left
        ...
        // computation of m_directionIntegers[k][l] for l>=degree_[k]
        // by recurrence relation
        for (k=1; k<m_dimensionality; k++) {
            unsigned int gk = degree[k];
            for (qm_int l=gk; l<c_bits; l++) {
                // eq. 8.19 "Monte Carlo Methods in Finance" by P. Jäckel
                qm_uns_long n = (m_directionIntegers[k][l-gk]>>gk);
                ...
                for (qm_Size j=1; j<gk; j++) {
                    // XORed with a selection of (unshifted) direction
                    // integers controlled by which of the a[k][j] are set
                    if ((ppmt[k] >> (gk-j-1)) & 1UL)
                        n ^= m_directionIntegers[k][l-j];
                }
                // a[k][gk] is always set, so m_directionIntegers[k][l-gk]
                // will always enter
                n ^= m_directionIntegers[k][l-gk];
                m_directionIntegers[k][l]=n;
            }
        }
        ...
        /* bool printDirectionIntegers = false;
           if (printDirectionIntegers) {
               ...
               for (k=0; k<std::min(32UL,m_dimensionality); k++) {
                   outStream << std::endl << k+1       << "\t"
                                          << degree[k] << "\t"
                                          << ppmt[k]   << "\t";
                   for (j=0; j<10; j++) {
                       outStream << io::power_of_two(
                           m_directionIntegers[k][j]) << "\t";
                   }
               }
               outStream.close();
           }
        */
        ...
        // initialize the Sobol sequence - first draw (not to be used for number generation
        for(k=0; k<m_dimensionality; k++) m_lastsequence[k] = m_directionIntegers[k][0];
}

	
	// Generate a new sequence
template<class RealType, int D>
inline void Sobol<RealType, D>::nextSequence() const  {
	seed_type n = this->sequenceCounter() + 1;
	QM_REQUIRE(n > 0, "Period exceeded: overflow");
    samples sam(m_dimensionality);
